...
describe("getQueryPerm", () => {
    ...
    test("supplies a query permission with given attributes", () => {
        ...
        const defaultQuery = {'sort_by': 'slug', 'order': 'asc', 'returning':['slug','description']}
        return getQueryPerm(tableName,allowedQueries,defaultQuery)
        .then((perm)=>{
            ...
            expect(perm.defaultQuery).toEqual({'sort_by': 'slug', 'order': 'asc','returning':['slug','description']})
    ...
    test("sets all allowed queries to include all column names when supplied with *", () => {
        const tableName = "topics"
        const allowedQueries = "*"
        const defaultQuery = {'sort_by': 'slug', 'order': 'asc', 'returning':['slug','description', 'img_url']}
        return getQueryPerm(tableName,allowedQueries,defaultQuery)
        .then((perm)=>{
            expect(perm.tableName).toEqual("topics")
            expect(perm.allowedQueries).toEqual({
...
describe("greenListQuery", () => {
    test("accepts empty query", () => {
        ...
        return getQueryPerm(tableName,allowedQueries,defaultQuery)
        .then((perm)=>{
            expect(perm.greenListQuery(query)).resolves.toEqual(undefined)
        }) 
    })
    ...
    test("rejects invalid search topic with correct 400 code and message", () => {
        ...
        return getQueryPerm(tableName,allowedQueries,defaultQuery)
        .then((perm)=>{
            expect(perm.greenListQuery(query)).rejects.toEqual({"msg":"Invalid search topic","status":400})
        }) 
    })
    test("rejects invalid search term with correct 400 code and message", () => {
        ...
        return getQueryPerm(tableName,allowedQueries,defaultQuery)
        .then((perm)=>{
            expect(perm.greenListQuery(query)).rejects.toEqual({"msg":"Invalid search term","status":400})
        }) 
    })
})
...
describe("completeQueryString", () => {
    test("completes string for empty query", () => {
        ...
        .then((perm)=>{
            expect(perm.completeQueryString(query)).toEqual("SELECT slug,description,img_url FROM topics")
        })
    })
    test("completes string for query with a sort_by", () => {
        ...
        return getQueryPerm(tableName,"*")
        .then((perm)=>{
            expect(perm.completeQueryString(query)).toEqual("SELECT slug,description,img_url FROM topics ORDER BY slug ")
        })
    })
    test("completes string for query with a sort_by and order", () => {
        ...
        return getQueryPerm(tableName,"*")
        .then((perm)=>{
            expect(perm.completeQueryString(query)).toEqual("SELECT slug,description,img_url FROM topics ORDER BY slug desc")
        })
    })
    test("completes string for query returning specific columns", () => {
        const tableName = "topics"
        const query = {"sort_by" : "slug", "order":"desc"}
        return getQueryPerm(tableName,"*",{"returning":["slug","description"]})
        .then((perm)=>{
            expect(perm.completeQueryString(query)).toEqual("SELECT slug,description FROM topics ORDER BY slug desc")
        })
    })
})
